...
#    `functionName` is a function's name in your module (e.g., `mpbPineEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/achubaty/Documents/GitHub/MPB/SpaDES/modules/mpbPine/tests/testthat`).
...
test_that("test Event1 and Event2.", {
  ...
  parameters <- list(
    ...
    mpbPine = list(.saveInitialTime = NA)
  )
  ...
  if (exists("mpbPineEvent1", envir = .GlobalEnv)) {
    simOutput <- mpbPineEvent1(mySim)
  } else {
    simOutput <- mySim$mpbPineEvent1(mySim)
  }
  ...
  if (exists("mpbPineEvent2", envir = .GlobalEnv)) {
    simOutput <- mpbPineEvent2(mySim)
  } else {
    simOutput <- mySim$mpbPineEvent2(mySim)
  }
